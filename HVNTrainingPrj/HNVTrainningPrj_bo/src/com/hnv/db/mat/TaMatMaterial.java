...
public class TaMatMaterial extends EntityAbstract<TaMatMaterial> {
	...
	public static final int	TYPE_04_NOD_DMZ 	= 50200; //serveur dmz
	...
	public static final int	TYPE_04_APP_EXTRA   = 10101; //Extranet
	//---------------------------------------------------------------------
	...
	public TaMatMaterial(String s, String commentTag){ //list machine
	...
	public void doMergeWith(TaMatMaterial ent) {
		...
		this.T_Info_11              = ent.T_Info_11;
		this.T_Info_12              = ent.T_Info_12;
		this.T_Info_13              = ent.T_Info_13;
		this.T_Info_14              = ent.T_Info_14;
		this.T_Info_15              = ent.T_Info_15;
...
//	} 
	...
	public void doBuildChildren(Integer levMax) throws Exception{
		...
		Session sess = TaMatMaterial.DAO.reqSessionCurrent();
		try {
			Set<Integer> ids = new HashSet<Integer>();
			ids.add(this.I_ID);
			this.O_Children = reqBuildChildren (sess, ids, levMax);
			if (sess!=null) TaMatMaterial.DAO.doSessionCommit(sess);
		}catch (Exception e) {
			if (sess!=null) TaMatMaterial.DAO.doSessionRollback(sess);
	...
	public static List<TaMatMaterialDetail> reqBuildStructDown(Session sess, Hashtable<Integer,EntityAbstract> mapPar, Integer levMax, Integer typDest) throws Exception{
		...
		List<TaMatMaterial> 		lstChild 	= TaMatMaterial.DAO.reqList_In(sess, TaMatMaterial.ATT_I_ID, setUids);
		Hashtable<Integer,EntityAbstract> mapChi	= ToolDBEntity.reqTabKeyInt(lstChild, TaMatMaterial.ATT_I_ID); 
		
		boolean canStop = false;
		for(TaMatMaterialDetail d: dets){
			Integer 		idPar 	= (Integer) d.req(TaMatMaterialDetail.ATT_I_MAT_MATERIAL_01);
			Integer 		idChild = (Integer) d.req(TaMatMaterialDetail.ATT_I_MAT_MATERIAL_02);
			TaMatMaterial	mPar	= (TaMatMaterial) mapPar.get(idPar); 
			TaMatMaterial 	mChi 	= (TaMatMaterial) mapChi.get(idChild);
	...
	public static List<TaMatMaterialDetail> reqBuildStructUp(Session sess, Hashtable<Integer,EntityAbstract> mapChi, Integer levMax, Integer typDest) throws Exception{
		...
		List<TaMatMaterial> 		lstPar 		= TaMatMaterial.DAO.reqList_In(sess, TaMatMaterial.ATT_I_ID, setUids);
		Hashtable<Integer,EntityAbstract> mapPar= ToolDBEntity.reqTabKeyInt(lstPar, TaMatMaterial.ATT_I_ID); 
	...
	public static List<TaMatMaterialDetail> reqBuildChildren(Session sess, Set<Integer> matIDs, Integer levMax) throws Exception{
		...
		List<TaMatMaterial> 		lstChild 	= TaMatMaterial.DAO.reqList_In(TaMatMaterial.ATT_I_ID, setUids);
		Hashtable<Integer,EntityAbstract> map 	= ToolDBEntity.reqTabKeyInt(lstChild, TaMatMaterial.ATT_I_ID); 
	...
	public static List<TaMatMaterialDetail> reqBuildParents(Session sess, Set<Integer> matIDs, Integer levMax) throws Exception{
		...
		List<TaMatMaterial> 		lstPar 		= TaMatMaterial.DAO.reqList_In(TaMatMaterial.ATT_I_ID, setUids);
		Hashtable<Integer,EntityAbstract> map 	= ToolDBEntity.reqTabKeyInt(lstPar, TaMatMaterial.ATT_I_ID); 
	...
	public JSONObject reqFavJson() {
		JSONObject 	o1 	= ToolJSON.reqJSonFromString(this.toString());
		JSONObject 	o2 	= new JSONObject();
		
		String []   attr = {"id", "name01", "code01", "inf01"};
		for (String s: attr) {
			o2.put(s, o1.get(s));
		}
