...
public class ServiceTpyDocument implements IService  {
	...
	private static List<TaTpyDocument> reqListNew(TaAutUser user, JSONObject json, String filePath) throws Exception {
		...
		for (int i=0; i<names.size(); i++){
			...
			TaTpyDocument doc		= new TaTpyDocument(fileTyp01, fileTyp02, fileTyp03, filesize, fName, fPath, new Date(),user.reqId(), entTyp, -1, stat, i);
			docs.add(doc);
		}
	...
	private static void doDel(TaAutUser user, JSONObject json, HttpServletResponse response) throws Exception {
		try{
			Integer 		id = json.containsKey("key") 	? Integer.parseInt(json.get("key").toString()) 	: null;
			if(id == null)	id = json.containsKey("id") 	? Integer.parseInt(json.get("id").toString()) 	: null;
	...
	private static boolean doDel(int id) throws Exception {
	...
	private static void doGet(TaAutUser user, JSONObject json, HttpServletResponse response) throws Exception {
		try{
			Integer id = json.containsKey("id") ? Integer.parseInt(json.get("id").toString()) : null;
			...
			if(!APIAuth.canAuthorize(user, DefRight.RIGHT_)) {
				API.doResponse(response,ToolJSON.reqJSonString(						
						DefJS.SESS_STAT, 1, 
						DefJS.SV_CODE		, DefAPI.SV_CODE_ERR_RIGHT
						));
				return;
			}
...
//			JSONArray arrFiles = ToolJSON.reqJSonArrayFromString((String) json.get("files"));
	...
	public static List<TaTpyDocument> reqNewtMod(TaAutUser user, Integer entTyp, Integer entId, JSONArray arrFiles) throws Exception {
		try{
			...
			if(!APIAuth.canAuthorize(user, DefRight.RIGHT_))	return null;
