...
public class ServiceTemplate implements IService {
	...
	public static final String SV_CLASS 				= "Service_".toLowerCase();	
...
//			ent.doBuildViewCount(true, objId);
	...
	private static void doNew(TaAutUser user,  JSONObject json, HttpServletResponse response) throws Exception  {
		...
		TaSysLock 	lock 	= ToolDBLock.reqLock (json, "lock", DefDB.DB_LOCK_NEW, DefDBExt.ID_TA_, (Integer)ent.req(TaNsoPost.ATT_I_ID), user.reqId(), user.reqStr(TaAutUser.ATT_T_LOGIN_01), null);
		API.doResponse(response,ToolJSON.reqJSonString(		//filter,
				DefJS.SESS_STAT		, 1, 
				DefJS.SV_CODE		, DefAPI.SV_CODE_API_YES,
				DefJS.RES_DATA		, ent,
	...
	private static TaNsoPost reqMod(TaAutUser user,  JSONObject json) throws Exception {
		...
		int stat 		= Integer.parseInt(obj.get("stat").toString());
		TaNsoPost ent 	= TaNsoPost.DAO.reqEntityByRef(objId);
		if (ent==null){
			return null;
		}
		...
		attr.remove(TaNsoPost.ATT_I_STATUS);
		attr.put(TaNsoPost.ATT_I_STATUS, stat);
		...
		case TaNsoPost.POST_TYPE_BLOG : 	
			...
			String prop04 	= ToolString.reqCovertStringToURL(title);
			attr.remove(TaNsoPost.ATT_T_PROPERTY_04);
		...
		//		Generate offer category:
		JSONArray		cats		= (JSONArray) obj.get("cats");
		if(cats!=null && cats.size() > 0) {
			List<TaTpyCategoryEntity> lstCat = null;
			lstCat =  TaTpyCategoryEntity.reqListMod(entTyp, objId, cats);
			ent.reqSet(TaNsoPost.ATT_O_CATS, lstCat);
		}
