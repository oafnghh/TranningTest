 ...
 * ----- Copyright 2017------------
 */
public class ServiceTpyInformation_Rissi implements IService {
	...
	public static final String SV_CLASS 		= "ServiceTpyInformation_Rissi".toLowerCase();	
	...
	public ServiceTpyInformation_Rissi(){
...
//
	...
	private static void doLstDyn(TaAutUser user,  JSONObject json, HttpServletResponse response) throws Exception {	
		...
		cri = Restrictions.and(cri, Restrictions.eq(TaTpyInformation.ATT_I_STATUS, stat));
...
//		Integer				active  = ToolData.reqInt(json, "active", 0);
...
//		attr.put(TaTpyInformation.ATT_I_ENTITY_ID, 0);
	...
	//---------------------------------------------------------------------------------------------------------
	//---------------------------------------------------------------------------------------------------------
	private static void doLckReq(TaAutUser user, JSONObject json, HttpServletResponse response) throws Exception  {
	...
	private static void doLckDel(TaAutUser user, JSONObject json, HttpServletResponse response) throws Exception  {
	...
	private static void doLckSav(TaAutUser user, JSONObject json, HttpServletResponse response) throws Exception  {
	...
	private static void doLckEnd(TaAutUser user, JSONObject json, HttpServletResponse response) throws Exception  {
	...
	private static void doChangeStat (TaTpyInformation ent) throws Exception {
		Integer stat = ent.reqInt(TaTpyInformation.ATT_I_STATUS);
		Integer typ  = ent.reqInt(TaTpyInformation.ATT_I_ENTITY_TYPE);
		Integer id   = ent.reqInt(TaTpyInformation.ATT_I_ENTITY_ID);
		
		if (stat == TaTpyInformation.STAT_OK) {
			List<TaTpyInformation> list = TaTpyInformation.DAO.reqList(Restrictions.eq(TaTpyInformation.ATT_I_ENTITY_TYPE, typ), Restrictions.eq(TaTpyInformation.ATT_I_ENTITY_ID, id), Restrictions.ne(TaTpyInformation.ATT_I_ID, ent.reqId()));
		
			if (list.size() > 0) {
				for (TaTpyInformation inf : list) {
					inf.reqSet(TaTpyInformation.ATT_I_STATUS, TaTpyInformation.STAT_WAITING);
				}
				
				TaTpyInformation.DAO.doMerge(list);
			}
