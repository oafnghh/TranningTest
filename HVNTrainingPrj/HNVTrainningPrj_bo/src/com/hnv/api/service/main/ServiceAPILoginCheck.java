...
//				}
			...
			}else if (method.equals(SIGNIN_SN)){ //---check synapse
				//----sign-on with synapse
				String 		idTok	= login;
				JSONObject  tok		= reqConnTokenSynapse (idTok);
				if (tok!=null) {
					JSONObject js 	= reqPayloadSynapse(tok.get("id_token").toString());
					//---check email and create account if not existing
					String uLogin	= (String)js.get("preferred_username");
					String email 	= (String)js.get("email");
					email			= email.replace("nomail", "");
					String name 	= (String)js.get("name");
					JSONArray grp	= (JSONArray)js.get("groups");
					user			= reqCheckAccountSN (uLogin, email, name, grp);
					if (user!=null){
						idTok 	= bcryptEncoder.encode(idTok);
						user. doHidePwd();
...
//				u.doBuildManager();
	...
	//--------------------------------------------------------------------------------------------
	private static JSONObject reqConnTokenSynapse(String code) throws Exception {
		String params = ToolURL.reqParamsString(
				"grant_type"	, "authorization_code",
				"client_id"		, DefAPIExt.SYNAPSE_CLI_ID,
				"client_secret"	, DefAPIExt.SYNAPSE_CLI_PWD,
				"redirect_uri"	, DefAPIExt.SYNAPSE_URL_REDIR,
				"code"			, code
				);
		
		String 	https_url 	= DefAPIExt.SYNAPSE_URL;
		String 	proxy_url 	= DefAPIExt.SYNAPSE_PROXY_URL;
		Integer proxy_port	= DefAPIExt.SYNAPSE_PROXY_PORT;
		String  https_header= "Content-Type:application/x-www-form-urlencoded;";
		
		String cont = ToolURL.reqHttpsRequest("POST", https_url, https_header, params, proxy_url, proxy_port, null, null);
		if (cont.indexOf("Err")>=0) return null;		
		JSONObject js = ToolJSON.reqJSonFromString(cont);
		return js;
	}
	
	private static JSONObject reqPayloadSynapse (String token){
		...
		JSONObject js = ToolJSON.reqJSonObjectFromString(payload) ;
		return js;
	}
	
	private static TaAutUser reqCheckAccountSN(String  uLogin, String email, String uName, JSONArray grp) throws Exception {
		Criterion cri 	= Restrictions.eq(TaAutUser.ATT_T_LOGIN_01, uLogin);
		...
		if(users != null && users.size()>0) {
			...
			TaAutUser ent	= TaAutUser.DAO.reqEntityByValue(TaAutUser.ATT_T_INFO_01, email);
			if (ent==null){
				return reqNewSN (uLogin, email, uName, grp);
			}
	...
	private static TaAutUser reqNewSN(String  uLogin, String email, String uName, JSONArray grp) throws Exception {
		Date 		now 		= new Date();
		//Create per
		Map<String, Object> attrPer = new HashMap(); 
		attrPer.put(TaPerPerson.ATT_D_DATE_01		, now);
		attrPer.put(TaPerPerson.ATT_D_DATE_02		, null);
		attrPer.put(TaPerPerson.ATT_I_TYPE_01		, TaPerPerson.TYP_01_NATURAL);
		attrPer.put(TaPerPerson.ATT_I_TYPE_02		, TaPerPerson.TYP_02_INTERNAL);
		attrPer.put(TaPerPerson.ATT_T_NAME_01		, uName);
		attrPer.put(TaPerPerson.ATT_T_INFO_03		, email);
		attrPer.put(TaPerPerson.ATT_I_STATUS_01		, TaPerPerson.STAT_01_ACTIVE_LV_01);
		attrPer.put(TaPerPerson.ATT_I_PER_MANAGER	, 1);
		...
		Map<String, Object> attrUsr = new HashMap();
		attrUsr.put(TaAutUser.ATT_T_LOGIN_01, uLogin);
		attrUsr.put(TaAutUser.ATT_T_PASS_01	, "");
		attrUsr.put(TaAutUser.ATT_T_INFO_01	, email);
		
		attrUsr.put(TaAutUser.ATT_T_INFO_04	, uName);
		attrUsr.put(TaAutUser.ATT_T_INFO_05	, grp!=null?grp.toJSONString():"[]");
		...
		if (grp!=null) {
			List<TaMatMaterial> myApps = new ArrayList<TaMatMaterial>();
			for (Object s : grp) {
				String grpName = "%" + s.toString().trim().replace("!SIT-MOE-", "").replace("!SIT-MOA-", "") + "%";
				List<TaMatMaterial> lst = TaMatMaterial.DAO.reqList(0,50,Restrictions.or(
											Restrictions.ilike(TaMatMaterial.ATT_T_INFO_02, grpName),
											Restrictions.ilike(TaMatMaterial.ATT_T_INFO_03, grpName)));
				if (lst!=null) myApps.addAll(lst);
				if (myApps.size()>100) break;
			}
			
			if (myApps.size()>0) {
				TaTpyFavorite fav = new TaTpyFavorite();
				fav.reqSet(TaTpyFavorite.ATT_I_AUT_USER		, ent.reqId());
				fav.reqSet(TaTpyFavorite.ATT_I_ENTITY_TYPE	, DefDBExt.ID_TA_MAT_MATERIAL_TYP01_10);
				fav.reqSet(TaTpyFavorite.ATT_I_ENTITY_ID	, 1);
				
				JSONObject 	favCont = new JSONObject();
				JSONArray	ids		= new JSONArray();
				JSONArray	lst		= new JSONArray();
				favCont.put("ids", ids);
				favCont.put("lst", lst);
				
				for (TaMatMaterial mat : myApps) {
					ids.add(mat.reqId());
					lst.add(mat.reqFavJson());
				}
				
				fav.reqSet(TaTpyFavorite.ATT_T_DESRCIPTION	, favCont.toJSONString());
				
				TaTpyFavorite.DAO.doPersist(fav);
			}
		}
