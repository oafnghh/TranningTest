...
import com.hnv.api.service.priv.mat.util.FileExport;
...
* ----- Copyright 2023------------
*/
public class ServiceMatMaterialPubl implements IService {
	...
	public static final String SV_CLASS 				= "ServiceMatMaterialPubl".toLowerCase();	
	...
	//---------------------------------------------------------------------------------------------------------
	//---------------------------------------------------------------------------------------------------------
	private static CacheData<TaMatMaterial> 		cache_entity= new CacheData<TaMatMaterial>		(500, DefTime.TIME_24_00_00_000, DefTime.TIME_00_30_00_000 );
	...
	private static void doGet(TaAutUser user,  JSONObject json, HttpServletResponse response) throws Exception  {	
		...
		TaMatMaterial 		ent 		= reqGet(entId, entCode01, entName01);
	...
	public static TaMatMaterial reqGet(Integer entId, String entCode01, String entName01) throws Exception{
		String				key		= entId + "-" + entCode01 + "-" + entName01;
		TaMatMaterial 		ent 	= cache_entity.reqData(key);	
		if (ent ==null) {
			if (entCode01!=null)
				ent 	= TaMatMaterial.DAO.reqEntityByValues(
						TaMatMaterial.ATT_I_ID			, entId, 
						TaMatMaterial.ATT_T_CODE_01		, entCode01
//						TaMatMaterial.ATT_I_STATUS_01	, TaMatMaterial.STAT_01_ACTIVE
						);
			else
				ent 	= TaMatMaterial.DAO.reqEntityByValues(
						TaMatMaterial.ATT_I_ID			, entId, 
						TaMatMaterial.ATT_T_NAME_01		, entName01
//						TaMatMaterial.ATT_I_STATUS_01	, TaMatMaterial.STAT_01_ACTIVE
						);
			...
			if (ent!=null) {
				...
				ent.doBuildStructTo(5, TaMatMaterial.TYPE_01_NOD);
...
//		}
	...
	private static void doLst(TaAutUser user,  JSONObject json, HttpServletResponse response) throws Exception  {
		...
		List<TaMatMaterial> 	list = reqLst(user, json); //and other params if necessary
		if (list==null){
			API.doResponse(response,DefAPI.API_MSG_KO);
			return;
		}
	...
	private static List<TaMatMaterial> reqLst(TaAutUser user, JSONObject json) throws Exception  {
		Integer 			nbLine      = ToolData.reqInt		(json, "nbLine" 	, 10);
		String 				searchkey	= ToolData.reqStr		(json, "searchkey"	, "");
		Integer				stat01		= ToolData.reqInt		(json, "stat01"		, TaMatMaterial.STAT_01_ACTIVE);
	...
	private static Criterion reqRestrictionList(
		...
		if (stat01 == null){
			stat01 = TaMatMaterial.STAT_01_ACTIVE; 
		}
		
		int colToSearch = 0; // => TaMatMaterial.ATT_T_NAME_01
		
		Criterion cri = Restrictions.eq(TaMatMaterial.ATT_I_STATUS_01, stat01);
		
		if (stat02!=null) {
			cri = Restrictions.and(	cri, Restrictions.eq(TaMatMaterial.ATT_I_STATUS_02, stat02));
		}
		
		if(typ01!=null && typ01>0) {
			colToSearch = typ01%10;
			typ01 		= (typ01/10)*10;
			cri 		= Restrictions.and(	cri, Restrictions.eq(TaMatMaterial.ATT_I_TYPE_01 , typ01));
		}
		if(typ02!=null && typ02>0) {
			cri = Restrictions.and(	cri, Restrictions.eq(TaMatMaterial.ATT_I_TYPE_02 , typ02));
		}

		if (searchKey!=null && searchKey.length()>0) {
			String colSearch 	= null;
			switch(colToSearch){
			case 0	: colSearch =	TaMatMaterial.ATT_T_NAME_01; searchKey = searchKey + "%"		; break;
			case 1	: colSearch =	TaMatMaterial.ATT_T_CODE_01; searchKey = "%"+ searchKey + "%"	; break;
			default	: colSearch =	TaMatMaterial.ATT_T_NAME_01;
			}
			
			if (typInf!=null && typInf>0) {
				switch(typInf){
				case 0	: colSearch =	TaMatMaterial.ATT_T_INFO_01; break;
				case 2	: colSearch =	TaMatMaterial.ATT_T_INFO_02; break;
				case 3	: colSearch =	TaMatMaterial.ATT_T_INFO_03; break;
				case 4	: colSearch =	TaMatMaterial.ATT_T_INFO_04; break;
				case 5	: colSearch =	TaMatMaterial.ATT_T_INFO_05; break;
				case 6	: colSearch =	TaMatMaterial.ATT_T_INFO_06; break;
				case 7	: colSearch =	TaMatMaterial.ATT_T_INFO_07; break;
				case 8	: colSearch =	TaMatMaterial.ATT_T_INFO_08; break;
				case 9	: colSearch =	TaMatMaterial.ATT_T_INFO_09; break;
				case 10	: colSearch =	TaMatMaterial.ATT_T_INFO_10; break;
				}
			}
	...
	private static void doLstByCol(TaAutUser user,  JSONObject json, HttpServletResponse response) throws Exception  {
		...
		List<TaMatMaterial> 	list = reqLstByCol(user, json); //and other params if necessary
		if (list==null){
			API.doResponse(response,DefAPI.API_MSG_KO);
			return;
		}
	...
	private static List<TaMatMaterial> reqLstByCol(TaAutUser user, JSONObject json) throws Exception  {
 		Integer 			nbLine      = ToolData.reqInt		(json, "nbLine" 	, 10);
		String 				searchkey	= ToolData.reqStr		(json, "searchkey"	, "");
		Integer				stat01		= ToolData.reqInt		(json, "stat01"		, TaMatMaterial.STAT_01_ACTIVE);
		Integer				typ01		= ToolData.reqInt		(json, "typ01"		, null);
...
//				cri = Restrictions.and(cri, Restrictions.eq(TaMatMaterial.ATT_I_TYPE_03, typ03));
//			}
			
			
			List<TaMatMaterial> list		= TaMatMaterial.DAO.reqList(begin, number, Order.asc(TaMatMaterial.ATT_T_NAME_01), cri);		
			
			if(withAvatar) {
				TaTpyDocument.doBuildTpyDocuments(list, 
						DefDBExt.ID_TA_MAT_MATERIAL, 
						TaTpyDocument.TYPE_01_FILE_MEDIA, 
						TaTpyDocument.TYPE_02_FILE_IMG_AVATAR, TaMatMaterial.ATT_O_DOCUMENTS, false);
			}

			if (total == 0 )	total		= TaMatMaterial.DAO.reqCount(cri).intValue();
...
//			cacheEnt_rs.reqCheckIfOld(keyWord); //cache in 2 hour
	...
	private static Set<Integer> STAT01_ALL = new HashSet<Integer>(){
		{
			add(TaMatMaterial.STAT_01_NEW);
			add(TaMatMaterial.STAT_01_ACTIVE);
			add(TaMatMaterial.STAT_01_ACTIVE_02);
			add(TaMatMaterial.STAT_01_ACTIVE_03);
			add(TaMatMaterial.STAT_01_REVIEW);
		}
	};
	private static Criterion reqRestrictionPagination(
		...
		Criterion cri = null;
		if (stat01 == null)
			cri = Restrictions.in(TaMatMaterial.ATT_I_STATUS_01, STAT01_ALL);
		else
			cri = Restrictions.eq(TaMatMaterial.ATT_I_STATUS_01, stat01);
		
		if (stat02!=null) {
			cri = Restrictions.and(	cri, Restrictions.eq(TaMatMaterial.ATT_I_STATUS_02, stat02));
		}
		if (stat03!=null) {
			cri = Restrictions.and(	cri, Restrictions.eq(TaMatMaterial.ATT_I_STATUS_03, stat03));
		}
		if (stat04!=null) {
			cri = Restrictions.and(	cri, Restrictions.eq(TaMatMaterial.ATT_I_STATUS_04, stat04));
		}
		if (stat05!=null) {
			cri = Restrictions.and(	cri, Restrictions.eq(TaMatMaterial.ATT_I_STATUS_05, stat05));
		}
		...
		if(typ04!=null) {
			cri = Restrictions.and(	cri, Restrictions.eq(TaMatMaterial.ATT_I_TYPE_04 , typ04));
		}
		if(typ05!=null) {
			cri = Restrictions.and(	cri, Restrictions.eq(TaMatMaterial.ATT_I_TYPE_05 , typ05));
		}
		...
		if (favIds!=null && favIds.size()>0) {
			cri = Restrictions.and(	cri, Restrictions.in(TaMatMaterial.ATT_I_ID , favIds));
		}
		return cri;
	}
	...
	private static Hashtable<String,String> mapColJsAtt = new Hashtable<String, String>(){
		{
			put("action", "");
			put("id"	, TaMatMaterial.ATT_I_ID );
			put("name01", TaMatMaterial.ATT_T_NAME_01 );
			put("name02", TaMatMaterial.ATT_T_NAME_02 );
			
			put("code01", TaMatMaterial.ATT_T_CODE_01 );
			put("code02", TaMatMaterial.ATT_T_CODE_02);
			put("code03", TaMatMaterial.ATT_T_CODE_03 );
			put("code04", TaMatMaterial.ATT_T_CODE_04 );
			
			put("stat"	, TaMatMaterial.ATT_I_STATUS_01 );
			put("stat01", TaMatMaterial.ATT_I_STATUS_01 );
			put("stat02", TaMatMaterial.ATT_I_STATUS_02 );
			put("stat03", TaMatMaterial.ATT_I_STATUS_03 );
			put("stat04", TaMatMaterial.ATT_I_STATUS_04 );
			put("stat05", TaMatMaterial.ATT_I_STATUS_05 );
			
			put("typ01", TaMatMaterial.ATT_I_TYPE_01 );
			put("typ02", TaMatMaterial.ATT_I_TYPE_02 );
			put("typ03", TaMatMaterial.ATT_I_TYPE_03 );
			put("typ04", TaMatMaterial.ATT_I_TYPE_04 );
			
			put("inf01" , TaMatMaterial.ATT_T_INFO_01 );
			put("inf02" , TaMatMaterial.ATT_T_INFO_02 );
			put("inf03" , TaMatMaterial.ATT_T_INFO_03 );
			put("inf04" , TaMatMaterial.ATT_T_INFO_04 );
			put("inf05" , TaMatMaterial.ATT_T_INFO_05 );
			put("inf06" , TaMatMaterial.ATT_T_INFO_06 );
			put("inf07" , TaMatMaterial.ATT_T_INFO_07 );
			put("inf08" , TaMatMaterial.ATT_T_INFO_08 );
			put("inf09" , TaMatMaterial.ATT_T_INFO_09 );
			put("inf10" , TaMatMaterial.ATT_T_INFO_10 );
		}
	};
	...
	private static void doLstDyn(TaAutUser user, JSONObject json, HttpServletResponse response) throws Exception {
		...
		Set<Integer>		typ04			= ToolData.reqSetInt	(json, "typ04"		, null);
		Set<Integer>		typ05			= ToolData.reqSetInt	(json, "typ05"		, null);
...
//		Integer 			manId	 		= null;
		...
		Criterion 	cri 			= reqRestriction(user, whereCond,  typ01, typ02, typ03, typ04, typ05, searchOptCol, searchOptOpe, searchKey);				
//		String criKey				= manId + "_" + searchKey.toString() + "_" + typ01 +"_" + typ02+"_" + stat; // get list voi du lieu trung voi du lieu dq tim trc

		//-----------------------------------------------------------------------------
		List<TaMatMaterial> lst 	= reqListDyn(dataTableOption, cri);
		if (lst==null ){
			API.doResponse(response, DefAPI.API_MSG_KO);
			return;
		}
	...
	private static Criterion reqRestriction(
			...
			JSONArray		whereCond	,
//			Set<Integer> 	stats		, 
			Set<Integer> 	typ01		, Set<Integer> 	typ02, Set<Integer> 	typ03, Set<Integer> 	typ04, Set<Integer> 	typ05,
			Set<String> 	searchCol	, String searchOptOpe	, Object sKey
			) throws Exception {	
		//--Pre-Check condition---------------------------------------------------
		Criterion cri =  Restrictions.ne(TaMatMaterial.ATT_I_STATUS_01 , TaMatMaterial.STAT_01_DELETED);
		...
		if(typ03!=null) {
			cri = Restrictions.and(	cri, Restrictions.in(TaMatMaterial.ATT_I_TYPE_03 , typ03));
		}
		if(typ04!=null) {
			cri = Restrictions.and(	cri, Restrictions.in(TaMatMaterial.ATT_I_TYPE_04 , typ04));
		}
		if(typ05!=null) {
			cri = Restrictions.and(	cri, Restrictions.in(TaMatMaterial.ATT_I_TYPE_05 , typ05));
		}
	...
	private static List<TaMatMaterial> reqListDyn(Object[] dataTableOption, Criterion cri) throws Exception {	
		...
		List<TaMatMaterial> list = new ArrayList<TaMatMaterial>();		

		Order 	order 	= null;	
		if (sortCol==null)
		 		sortCol = TaMatMaterial.ATT_T_NAME_01;; // sap xep 
	...
	private static Number reqNbListDyn(Set<Integer> typ01) throws Exception {	
		if (typ01!=null){
			Criterion cri = Restrictions.in(TaMatMaterial.ATT_I_TYPE_01, typ01);
			return TaMatMaterial.DAO.reqCount(cri);		
		}
		
		return TaMatMaterial.DAO.reqCount();	
	}
	...
	private static void doLstDynSQL(TaAutUser user, JSONObject json, HttpServletResponse response) throws Exception {
		...
		//-----------------------------------------------------------------------------
		List<TaMatMaterial> lst 	= reqListDynSQL(dataTableOption, sql);
		if (lst==null ){
			API.doResponse(response, DefAPI.API_MSG_KO);
			return;
		}
	...
	private static String cond_join_cat 		= DefDBExt.TA_MAT_MATERIAL 			+ " m inner join " 
												...
												+ "     c."+  TaTpyCategoryEntity.COL_I_ENTITY_ID    + " = m." + TaMatMaterial.COL_I_ID 	
												+ " and c." + TaTpyCategoryEntity.COL_I_ENTITY_TYPE  + " = "   + DefDBExt.ID_TA_MAT_MATERIAL 
												+ " and c." + TaTpyCategoryEntity.COL_I_TPY_CATEGORY + " = %d";
	
	private static String cond_where_stat01 	= "m." + TaMatMaterial.COL_I_STATUS_01 + " in %s";
	private static String cond_where_typ01 		= "m." + TaMatMaterial.COL_I_TYPE_01 + " in %s";
	
	private static String cond_where_filter_00 	= "(lower(m." + TaMatMaterial.ATT_T_NAME_01 + ") like  lower('%s') or " //1
												+  "lower(m." + TaMatMaterial.ATT_T_CODE_01 + ") like  lower('%s') or "  //2
												+  "lower(m." + TaMatMaterial.ATT_T_INFO_01 + ") like  lower('%s') or "  //3
												+  "lower(m." + TaMatMaterial.ATT_T_INFO_02 + ") like  lower('%s') or "  //4
												+  "lower(m." + TaMatMaterial.ATT_T_INFO_03 + ") like  lower('%s') or "  //5
												+  "lower(m." + TaMatMaterial.ATT_T_INFO_04 + ") like  lower('%s') or "  //6
												+  "lower(m." + TaMatMaterial.ATT_T_INFO_05 + ") like  lower('%s') or "  //7
												+  "lower(m." + TaMatMaterial.ATT_T_INFO_06 + ") like  lower('%s') or "  //8
												+  "lower(m." + TaMatMaterial.ATT_T_INFO_07 + ") like  lower('%s') or "  //9
												+  "lower(m." + TaMatMaterial.ATT_T_INFO_08 + ") like  lower('%s') or "  //10
												+  "lower(m." + TaMatMaterial.ATT_T_INFO_09 + ") like  lower('%s') or "  //11
												+  "lower(m." + TaMatMaterial.ATT_T_INFO_10 + ") like  lower('%s') ) ";  //12
	private static String cond_where_filter_01 	= "(lower(m." + TaMatMaterial.ATT_T_NAME_01 + ") like  lower('%s') ) "; //1
	private static String cond_where_filter_02 	= "(lower(m." + TaMatMaterial.ATT_T_CODE_01 + ") like  lower('%s') ) "; //1
	...
	private static List<TaMatMaterial> reqListDynSQL(Object[] dataTableOption, String sql) throws Exception {	
		Set<String>		searchKey	= (Set<String>)	dataTableOption[0];	
		int 			begin 		= (int)			dataTableOption[1];
		int 			number 		= (int)			dataTableOption[2]; 
		String 			sortCol 	= (String)		dataTableOption[3]; 
		...
		list	= TaMatMaterial.DAO.reqList(begin, number, sql);
		return list;
	}

	private static Number reqNbListDynSQL(String sql) throws Exception {	
		return TaMatMaterial.DAO.reqCount(sql);	
	}
...
//		Set<Integer>		cat				= ToolData.reqSetInt	(json, "catIds"		, null);
		...
		Criterion 	cri 			= reqRestriction(user, whereCond,  typ01, typ02, typ03, typ04, typ05, null, null, null);	
//		cri = Restrictions.and(	cri, Restrictions.in(TaMatMaterial.ATT_O_CATS , cat));

//		String criKey				= manId + "_" + searchKey.toString() + "_" + typ01 +"_" + typ02+"_" + stat; // get list voi du lieu trung voi du lieu dq tim trc

		//-----------------------------------------------------------------------------
		List<TaMatMaterial> lst 	= reqListExp(cri);
...
//		Call exportFile
		...
		FileExport.doExport(lst, colExport, colLab, pathFile+fileName);
	...
	private static List<TaMatMaterial> reqListExp(Criterion cri) throws Exception {		
		List<TaMatMaterial> list = new ArrayList<TaMatMaterial>();		
