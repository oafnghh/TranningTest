define([
	'jquery',
	'text!template/per/partner/PerPartner_Ent_Tab_Report_Producer.html'
	],
	function($,
			PerPartner_Ent_Tab_Report_Producer
	) {

	var PerPartnerEntTabReportProducer     = function (header,content,footer, grpName) {
		...
		this.do_lc_show		= function(obj, mode){
			...
			pr_parentId 			= obj.id;
			try{
				tmplCtrl.do_lc_put_tmpl(tmplName.PERPARTNER_ENT_TAB_REPORT_PRODUCER, PerPartner_Ent_Tab_Report_Producer); 	
				$("#div_PerPartner_Ent_Tab_Report").html(tmplCtrl.req_lc_compile_tmpl(tmplName.PERPARTNER_ENT_TAB_REPORT_PRODUCER, {}));
				do_get_producer_mat_lst(obj);
//				do_get_producer_inv_in_detail_lst_dyn(obj);
//				do_get_producer_inv_out_detail_lst_dyn(obj);
			}catch(e) {
				do_gl_show_Notify_Msg_Error("PerPartnerEntTabReportProducer: " + e.toString());
				do_gl_exception_send(App.path.BASE_URL_API_PRIV,  "per.partner", "PerPartnerEntTabReportProducer", "do_lc_show", e.toString()) ;
			}
		};
		...
		function do_get_producer_mat_lst(obj){
			var ref 			= {};
			ref[svClass] 		= "ServicePerPerson"; 
			ref[svName]			= "SVPerProducerMatMaterialLst"; 
			ref[userId]			= App.data.user.id;
			ref[sessId]			= App.data.session_id;
			ref["producerId"]	= obj.id;
		...
		function do_show_producer_mat_lst(sharedJson){
			if(sharedJson[App['const'].SV_CODE] == App['const'].SV_CODE_API_YES) {
				...
				req_gl_create_datatable(obj, "#div_PerPartner_Ent_Tab_report_Producer_Mat", additionalConfig, pr_default_new_line_mat, function(){
...
//						}
		...
		function do_get_producer_inv_in_detail_lst_dyn(obj){
			var ref 			= {};
			ref[svClass] 		= "ServicePerPerson"; 
			ref[svName]			= "SVPerProducerInvDetLstDyn"; 
			ref[userId]			= App.data.user.id;
			ref[sessId]			= App.data.session_id;
			ref["producerId"]	= obj.id;
			...
			var div ="#div_PerPartner_Ent_Tab_report_Producer_Inv_In_Detail";
			...
			var fError 		= req_gl_funct(App, do_gl_show_Notify_Msg, [$.i18n("common_err_ajax")]);	
			var oTable 		= req_gl_Datatable_Ajax_Dyn		(div, App.path.BASE_URL_API_PRIV,App.data["HttpSecuHeader"], filename, colConfig, ref, fError, 1000, null, null, null , { "order": [[ 0, "desc" ]]}, false);
		}
		
		function do_get_producer_inv_out_detail_lst_dyn(obj){
			var ref 			= {};
			ref[svClass] 		= "ServicePerPerson"; 
			ref[svName]			= "SVPerProducerInvDetLstDyn"; 
			ref[userId]			= App.data.user.id;
			ref[sessId]			= App.data.session_id;
			ref["producerId"]	= obj.id;
			ref["typ02"]		= 2; //typ02 = 1: hóa đơn nhập; 2: hóa đơn xuất
			
			var lang = localStorage.language;
			if (lang ==null ) lang = "en";
			var div ="#div_PerPartner_Ent_Tab_report_Producer_Inv_Out_Detail";
...
//			var oTable 	= $(div).find('.table-datatableDyn').dataTable();
	...
	return PerPartnerEntTabReportProducer;
});