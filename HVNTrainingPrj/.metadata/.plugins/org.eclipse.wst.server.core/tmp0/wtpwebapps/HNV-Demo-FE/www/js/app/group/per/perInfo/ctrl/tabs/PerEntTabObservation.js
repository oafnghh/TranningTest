define([
	'jquery', 
	'text!template/shp/per/partner/PerPartner_Ent_Tab_Observation.html',
	'text!template/shp/per/common/Per_Sel_List_Address.html'
	],
	function($,
			PerPartner_Ent_Tab_Observation,
			Per_Sel_List_Address
	) {

	var PerPartnerEntTabObservation     = function (header,content,footer, grpName) {
		...
		var pr_default_new_line	= {
				id 		: null,
				lab		: App.data.user.per.name01 +" "+App.data.user.per.name02,
				typ01	: 3,
				typ02	: null,
				info03	: null,
				info04 	: null,
		...
		this.do_lc_init		= function(){
			...
			tmplName.PER_ENT_TAB_OBSERVATION_LIST		= "PerPartner_Ent_Tab_Observation";
...
//				do_gl_show_Notify_Msg_Error("PerPartnerEntTabAddress: " + e.toString());
		...
		var do_bind_event = function (obj, mode){
			...
			req_gl_create_datatable(obj, "#tab_Per_Ent_Tab_Observation_List", additionalConfig, pr_default_new_line, function(){
				if(pr_mode == App['const'].MODE_MOD || pr_mode == App['const'].MODE_NEW) {
					$("#div_Per_Ent_Tab_Observation_List").find("#btn_add").removeAttr("disabled");
				}
				$("#div_Per_Ent_Tab_Observation_List").find(".a_delete ").hide();
			});

		}.bind(this);

	}
	return PerPartnerEntTabObservation;
});