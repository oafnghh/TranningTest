define([
        'jquery',
        'text!template/per/partner/PerPartner_Ent_Btn.html'
        ],
	...
	var PerPartnerEntBtn     = function (header,content,footer, grpName) {
		...
		this.do_lc_init		= function(partnerType){
			...
			tmplCtrl.do_lc_put_tmpl(tmplName.PER_ENT_BTN, PerPartner_Ent_Btn); 			
		}
		...
		this.do_lc_show		= function(obj, mode){
			try{
				...
				do_gl_show_Notify_Msg_Error("PerPartnerEntBtn: " + e.toString());
				do_gl_exception_send(App.path.BASE_URL_API_PRIV,  "per.partner", "PerPartnerEntBtn", "do_lc_show", e.toString()) ;
			}
		};
		...
		function do_bind_event_btn_create(){
			...
			$("#btn_PerPartner_create").click(function(){
				var newObj		= {"typ02" : self.lc_partner_type};
...
//		}
		...
		function do_bind_event_btn_del(obj){
			...
			$("#btn_PerPartner_del").click(function(){
				var typePartner = obj.typ02;
				var rightCode = rights.req_lc_Right(typePartner, RIGHT_D);
				if(rightCode == -1){
					do_gl_show_Notify_Msg_Error($.i18n("user_not_support_D_" + typePartner));
					return;
				}
		...
		function do_bind_event_btn_save(obj, mode){
			...
			$("#btn_PerPartner_save").click(function(){
				if(App.data.mode == App['const'].MODE_NEW) {	
					var typePartner = parseInt($("#sel_list_type_partner").val());
					var rightCode = rights.req_lc_Right(typePartner, RIGHT_N);
					if(rightCode == -1){
						do_gl_show_Notify_Msg_Error($.i18n("user_not_support_N_" + typePartner));
						return;
					}
				}
...
//		
		var do_Enabled_Edit = function(){
			...
			$("#dtNew").attr("disabled","disabled");
			$("#dtMod").attr("disabled","disabled");
		}
