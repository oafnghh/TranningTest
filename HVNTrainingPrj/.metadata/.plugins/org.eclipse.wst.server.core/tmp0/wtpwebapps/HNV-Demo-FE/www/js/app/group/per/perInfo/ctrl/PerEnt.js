	...
	var PerEnt     = function (header,content,footer, grpName) {
		...
		this.do_lc_show		= function(obj, mode, typePartner){			
			...
			if (!obj){
				...
				if(mode !=  App['const'].MODE_NEW) pr_ctr_EntTabs		.do_lc_show(obj, mode, typePartner);
			...
			if(mode == App['const'].MODE_NEW || mode == App['const'].MODE_MOD) {
				...
				if($('#btn_add').length) $("#btn_add").prop( "disabled", false);

				// Remove disabled attr of filter box
				$('#inp_order_ent_dt01').removeAttr('disabled');
				$('#inp_order_ent_tm01').removeAttr('disabled');
				$('#inp_order_ent_dt02').removeAttr('disabled');
				$('#inp_order_ent_tm02').removeAttr('disabled');
				$('#inp_ord_mat_id').removeAttr('disabled');
				$('#inp_ord_mat_code').removeAttr('disabled');
			}
		}
...
//			if (per.data.files && per.data.logo) 	per.data.files.push(per.data.logo);
		...
		var do_send_new = function(per, typ) {
			var mode			= null;
			per.data.typ01		= typePersonMoral;
			per.data.typ02		= typeCompany;
		...
		function do_refresh_list (sharedJSon, localObj) {
			if(sharedJSon[App['const'].SV_CODE] == App['const'].SV_CODE_API_YES) {
				var per = sharedJSon[App['const'].RES_DATA];
				var typPartner = per && per.typ02 != null? per.typ02 : localObj.typ02;
