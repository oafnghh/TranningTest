define([
        'jquery',
        'text!template/per/societe/PerSociete_Ent_Tab_Doc.html',
     
        ],
        function($,
        		PerSociete_Ent_Tab_Doc
        		) {
	...
	var PerPartnerEntTabDoc     = function (header,content,footer, grpName) {
		...
		this.do_lc_show		= function(obj, mode){
			...
			try{
				tmplCtrl.do_lc_put_tmpl(tmplName.PER_SOCIETE_ENT_TAB_DOC, PerSociete_Ent_Tab_Doc);
				$("#div_PerSociete_Ent_Tab_Doc").html(tmplCtrl.req_lc_compile_tmpl(tmplName.PER_SOCIETE_ENT_TAB_DOC, obj));
				do_bind_event();
			}catch(e) {
				do_gl_show_Notify_Msg_Error("PerSocieteEntTabDoc " + e.toString());
				do_gl_exception_send(App.path.BASE_URL_API_PRIV,  "per.partner", "PerSocieteEntTabDoc", "do_lc_show", e.toString()) ;
			}
		};
